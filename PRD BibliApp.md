...
#### 📑 Bookmarks (favoritos/destaques/notas)
- Tabela `bookmarks` no Supabase com RLS (auth.uid() = user_profile_id) para CRUD seguro.
- Tipos (`bookmark_type` enum): `verse`/`highlight` (versículo favoritado/destacado), `note` (anotação por versículo), `devotional` (devocional favoritado).
- Unicidade esperada: 1 registro por usuário + tipo + verse_id (evita duplicar destaque/nota do mesmo versículo). Recomenda-se índice único (user_profile_id, bookmark_type, verse_id).
- `verse_id` referencia `verses`; `highlight_color` guarda cor hex da paleta; `note_text` obrigatório quando `bookmark_type = 'note'`.
- Listagem com filtro/paginação por tipo; remoção/desfavoritar suportada.
